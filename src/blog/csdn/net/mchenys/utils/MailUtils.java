...
public class MailUtils {
	...
	public static void sendMail(String email, String emailMsg) throws AddressException, MessagingException {
		...
		// 设置发送邮件的服务器
		props.setProperty("mail.host", "localhost");// 本地服务器写localhost即可,网络服务器要写正确,例如QQ的smtp.qq.com
		props.setProperty("mail.smtp.auth", "true");// 指定验证为true

		// 创建验证器
		Authenticator auth = new Authenticator() {
			public PasswordAuthentication getPasswordAuthentication() {
				// 设置发送人的帐号(帐号不用带后缀)和密码
		...
		// 设置邮件内容
		message.setContent(emailMsg, "text/html;charset=utf-8");//或者message.setText(emailMsg);
		
		// 3.创建 Transport用于将邮件发送
		Transport.send(message);
		
		//或者这样,可以发送给所有接收者(To,Cc,Bcc等 )
